...
'mluo@merkleinc.com
'Version:
'2015-5-04 intail version
'----------------------------------

Public Sub Sub_Edit_Jnr(xmlDom As MSXML2.DOMDocument, jnr_name As String)
On Error GoTo FATAL_ERROR
     ...
     Set chlNodeList = Nothing
        ...
        For Each xmlNode In xmlNodeList
          If xmlNode.attributes.getNamedItem("NAME").nodeValue = jnr_name Then
              Set chlNodeList = xmlNode.childNodes
                 For Each chlNode In chlNodeList
                    If chlNode.nodeName = "TRANSFORMFIELD" Then
                        ...
                        ActiveSheet.Range("H" & output_at_row).Value = port_type
        ...
        If chlNodeList Is Nothing Then
            Call Sub_OkOnly_Msgbox("Can not find specified joiner '" + jnr_name + "'.")
            Exit Sub
        End If
        ...
        Call Sub_Hint_Box_Add(Format(Time, "hh:mm:ss") + ": You are editing " + jnr_name + " and its port layout has displayed at right.You can modify these ports as you want, then click 'Update This Transformation' to save changes." + vbLf)
        Call Sub_Hint_Box_Add("------------------------------------------------------" + vbLf)
        
        Exit Sub
FATAL_ERROR:
    Call Sub_Error_Handle("Sub_Edit_Jnr")
...
'2015-5-04 intail version
'----------------------------------
Public Sub Sub_Update_Jnr(xmlDom As MSXML2.DOMDocument, jnr_name As String)
On Error GoTo FATAL_ERROR
     ...
     Dim fieldNodeList As MSXML2.IXMLDOMNodeList
            ...
            For Each xmlNode In xmlNodeList
                ...
                If xmlNode.attributes.getNamedItem("NAME").nodeValue = jnr_name Then
                    ...
                    For output_at_row = 10 To end_at_row
                        ...
                        port_type = ActiveSheet.Range("H" & output_at_row).Value
                        ...
                        Case "INPUT/OUTPUT", "INPUT/OUTPUT/MASTER"
                        Case Else
                        ...
                        If chlNode.nodeName <> "TRANSFORMFIELD" Then
                            Set newNode = xmlDom.createElement("TRANSFORMFIELD")
                                    ...
                                    Set jnr_attr = xmlDom.createAttribute("DATATYPE")
                                    jnr_attr.Value = port_datatype
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("DEFAULTVALUE")
                                    jnr_attr.Value = ""
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("DESCRIPTION")
                                    jnr_attr.Value = ""
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("NAME")
                                    jnr_attr.Value = port_name
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("PICTURETEXT")
                                    jnr_attr.Value = ""
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("PORTTYPE")
                                    jnr_attr.Value = port_type
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("PRECISION")
                                    jnr_attr.Value = port_pre
                                    newNode.attributes.setNamedItem (jnr_attr)

                                    Set jnr_attr = xmlDom.createAttribute("SCALE")
                                    jnr_attr.Value = port_scale
                                    newNode.attributes.setNamedItem (jnr_attr)
            ...
            Call Sub_Hint_Box_Add(Format(Time, "hh:mm:ss") + ": Port changes for " + jnr_name + " have been updated to the XML file." + vbLf)
            Call Sub_Hint_Box_Add("------------------------------------------------------" + vbLf)
        
            Exit Sub
FATAL_ERROR:
    Call Sub_Error_Handle("Sub_Update_Jnr")
End Sub
