...
'----------------------------------
Public reuseable_flg As String
'0-----nonreuseable
'1-----reuseable

Public Sub Sub_Edit_Exp(xmlDom As MSXML2.DOMDocument, exp_name As String)
On Error GoTo FATAL_ERROR
     ...
     If InStr(exp_name, "(") = 0 Then
        reuseable_flg = 0
        Set xmlNodeList = xmlDom.selectNodes("//POWERMART/REPOSITORY/FOLDER/MAPPING/TRANSFORMATION")
     Else
        reuseable_flg = 1
        exp_name = Mid(exp_name, InStr(exp_name, "(") + 1, Len(exp_name) - InStr(exp_name, "(") - 1)
     ...
     Set chlNodeList = Nothing
        ...
        For Each xmlNode In xmlNodeList
          If xmlNode.attributes.getNamedItem("NAME").nodeValue = exp_name Then
        ...
        If chlNodeList Is Nothing Then
            Call Sub_OkOnly_Msgbox("Can not find specified expression '" + exp_name + "'.")
            Exit Sub
        End If
        ...
        Call Sub_Hint_Box_Add(Format(Time, "hh:mm:ss") + ": You are editing " + exp_name + " and its port layout has displayed at right.You can modify these ports as you want, then click 'Update This Transformation' to save changes." + vbLf)
        Call Sub_Hint_Box_Add("------------------------------------------------------" + vbLf)
        
        Exit Sub
FATAL_ERROR:
    Call Sub_Error_Handle("Sub_Edit_Exp")
End Sub
'----------------------------------
'mluo@merkleinc.com
'Version:
'2015-4-12 intail version
'----------------------------------
Public Sub Sub_Update_Exp(xmlDom As MSXML2.DOMDocument, exp_name As String)
On Error GoTo FATAL_ERROR
     ...
     comment_check_flg = 1
            ...
            Call Sub_Hint_Box_Add(Format(Time, "hh:mm:ss") + ": Port changes for " + exp_name + " have been updated to the XML file." + vbLf)
            Call Sub_Hint_Box_Add("------------------------------------------------------" + vbLf)
        
            Exit Sub
FATAL_ERROR:
    Call Sub_Error_Handle("Sub_Update_Exp")
End Sub
